def install(pid,vmid=None,chost_id=None,dhost_id=None,parameters=None):
	...
	(newmac,newip,vncport)=new_mac_ip(vmcount)
        ...
        if namerepcheck(vmname) == False:
            ...
            if not os.path.exists (getconstant('vmfiles_path')+'/'+vmname):
	    ##if not os.path.exists('/mnt/baadalcse/'+vmname):
                os.makedirs(getconstant('vmfiles_path')+'/'+vmname)
		##os.makedirs('/mnt/baadalcse/'+vmname)
            vm_loc = getconstant('vmfiles_path')+'/'+vmname+'/'+ vmname + '.qcow2'
	    ##vm_loc = '/mnt/baadalcse/'+vmname+'/'+vmname+'.qcow2'
	    if(not ifValidVM(req.cloneparentname)):
		template = db(db.template.id == req.templateid).select()[0]
		template_hdfile = template.hdfile
		template_loc=getconstant('vmfiles_path')+'/'+getconstant('templates_path')+template_hdfile
		##template_loc='/mnt/baadalcse/vm_template/'+template_hdfile
		print "Copy and Move in progress"
		...
		##command='time cp '+getconstant('vmfiles_path')+'/'+getconstant('templates_path')+template_hdfile+' '+getconstant('vmfiles_path')+'/'+getconstant('templates_path')+'tmp' 
		print command
		a=commands.getstatusoutput(command) #--S
		...
		##command='sudo ssh '+datastore.ip+' mv '+datastore.path+'/'+getconstant("templates_dir")+'/tmp/'+template_hdfile+' '+datastore.path+'/'+vmname+'/'+vmname+'.qcow2'
                command='mv '+getconstant('vmfiles_path')+'/'+getconstant('templates_path')+'tmp'+'/'+template_hdfile+' '+getconstant('vmfiles_path')+'/'+vmname+'/'+vmname+'.qcow2'
		print command
		myparent=""
		a=commands.getstatusoutput(command) #--S
		print a
		errormsg=errormsg+a[1]
            ...
            if(ifValidVM(req.cloneparentname)):
                ...
                print "Copy and Move in progress"
                command='sudo ssh '+datastore.ip+' ndmpcopy -sa '+origvm.datastore.username+':'+origvm.datastore.password+' '+origvm.datastore.ip+':'+origvm.datastore.path+'/'+origvm.name+'/'+origvm.name+'.qcow2 '+datastore.path+'/'+getconstant("templates_dir")+'/tmp'
                print command
		...
		print a
               # print commands.getstatusoutput(command)
		errormsg = errormsg + a[1]
            (ram,vcpus)=computeeffres(req.RAM,req.vCPUs,1)
		...
		##send_confirmation_email(user_id,faculty,vm_name) ----NEEDS TO BE CORRECTED
