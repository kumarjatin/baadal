...
def migrate(pid,vmid,chost_id,dhost_id,parameters):
    ...
    #we first try Live Migration, If it fails then we do Dead Migration.
    import commands
...
#	print "Suspending VM"
#	command= 'virsh --connect qemu+ssh://root@'+chost+'/system suspend'+vm.name
#	print command
#	print commands.getstatusoutput(command)
#	print commands.getstatusoutput('virsh --connect qemu+ssh://root@'+chost+'/system migrate '+vm.name+'qemu+ssh://root@'+dhost+'/system')
#	command= 'virsh --connect qemu+ssh://root@'+dhost+'/system resume '+vm.name	
#	output[0] = commands.getstatusoutput(command)
#	print output[0]
#	if(output[0]==0):
#            db(db.vm.id == vmid).update(hostid = dhost_id)
#            updatevmcount(chost_id,dhost_id)
...
#def deadmigrate(pid,vmid,chost_id,dhost_id,parameters)
#Dead migration
def deadMigrate(pid, vmid, chost_id, dhost_id, parameters):
    print chost_d
    print dhost_id
    vm = db(db.vm.id == vmid).select()[0]
    #chost = db(db.vm.id = vmid).select(db.vm.hostid)[0].hostid.ip  #IP address of our initial/source host
    print chost

    import commmands
    dhost = db(db.host.id == dhost_id).select()[0].ip #IP address of destination host
    #We already have backup of xml file in /root/migratexml of Baadal while trying live migration
    print "Suspending VM for dead migration"
    command = 'virsh --connect qemu+ssh://root@' + chost + '/system suspend ' + vm.name
    print commands.getstatusoutput(command)
    print "Migrating ..."
    command = 'virsh --connect qemu+ssh://root@' + chost + '/system migrate ' + vm.name + 'qemu+ssh://root@' + dhost + '/system'
    print command
    output = commands.getstatusoutput(command)
    print output[0]
    print output[1]
    ...
    if(output[0] == 0):
        #Migration is successful, resume the vm.
        print "Resuming VM after migration"
        command = 'virsh --connect qemu+ssh://root@' + dhost + '/system resume ' + vm.name
        print command
        #Check for resume ???
        print commands.getoutputstatus(command)
