...
def hostpoweroperation():
    ...
    try:
        ...
        if(freehostscount==2):print "Everything is Balanced. Green Cloud :)"
            ...
            newhosts=db(db.host.status==0).select()[0:(2-freehostscount)] #Select only Shutoff hosts
            for host in newhosts:
                print "Sending magic packet to "+host.name
                print commands.getstatusoutput("ssh root@"+masterhost+" wakeonlan "+str(host.mac))
        elif(freehosts>2):
