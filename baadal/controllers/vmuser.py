...
def list():
    try:
        ...
        vmlist=[]
        for vm in vms:                  
	    vm=db(db.vm.id==vm.vmid).select()[0]
	    addtocost(vm.name)
...
def migrate_vm():
    ...
    try:
        requires_moderator()
        import string, os
        form_fields = ['vm','chost','dhost','live','suspend']
        form_labels = {'vm':'VM Name','chost':'Current Host','dhost':'Destination Host', 'live':'Live', 'suspend':'Suspend'}
        ...
        if form.accepts(request.vars, session):
            ...
            if(form.vars.live): options = options + "--live "
            options = options + "--persistent --undefinesource "
