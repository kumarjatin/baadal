...
Normal_pkg_lst=(ssh zip unzip tar openssh-server build-essential python2.7:python2.5 python-dev python-paramiko apache2 libapache2-mod-wsgi postfix wget libapache2-mod-gnutls apache2.2-common python-matplotlib python-reportlab mercurial libvirt-bin python-libvirt sshpass)
...
Cnfgr_Ldap_Srvc()
{
	...
	cp ldap_krb/krb5.conf /etc/krb5.conf -f
	mv /etc/ldap.conf /etc/ldap.conf.bkp
	cp ldap_krb/ldap.conf /etc/ldap.conf -f
	mv /etc/nsswitch.conf /etc/nsswitch.conf.bkp
	cp ldap_krb/nsswitch.conf /etc/nsswitch.conf -f
	mv /etc/pam.d/common-account /etc/pam.d/common-account.bkp
	cp ldap_krb/common-account /etc/pam.d/common-account -f
	mv /etc/pam.d/common-auth /etc/pam.d/common-auth.bkp
	cp ldap_krb/common-auth /etc/pam.d/common-auth -f
	mv /etc/pam.d/common-password /etc/pam.d/common-password.bkp
	cp ldap_krb/common-password /etc/pam.d/common-password -f
	mv /etc/pam.d/common-session /etc/pam.d/common-session.bkp
	cp ldap_krb/common-session /etc/pam.d/common-session -f
}
...
Setup_Web2py()
{
	...
	cp -r host/ /home/www-data/web2py/applications/baadal/host/
...
#Function to configure Password-Less SSH with the host
Configure_Pwdless_Ssh()
{
	...
	host_ip_list=(`echo $host_ip_list | tr "," " "`)

	for ip in ${host_ip_list[@]}; do

		ip=(`echo $ip | tr ":" " "`)

		echo "Establishing passwordless ssh with ${ip[0]}"		
		su www-data -c "sshpass -p ${ip[1]} ssh -o StrictHostKeyChecking=no root@${ip[0]} 'mkdir -p .ssh'"
		su www-data -c "cat /var/www/.ssh/id_rsa.pub | sshpass -p ${ip[1]} ssh root@${ip[0]} 'cat >> .ssh/authorized_keys'"
		su www-data -c "scp /home/www-data/web2py/applications/baadal/host/check_host_status.py root@${ip[0]}:"
		su www-data -c "scp /home/www-data/web2py/applications/baadal/host/config-file-host root@${ip[0]}:"
		su www-data -c "scp /home/www-data/web2py/applications/baadal/host/install_host.sh root@${ip[0]}:"
		su www-data -c "scp /home/www-data/web2py/applications/baadal/host/bridge root@${ip[0]}:"
		su www-data -c "ssh root@${ip[0]} 'sh install_host.sh ; cp /etc/network/interfaces /etc/network/interfaces_orig; cat bridge /etc/network/interfaces > /etc/network/interfaces; /etc/init.d/networking restart;'"
		su www-data -c "ssh root@${ip[0]} 'brctl stp br0 on; mkdir -p /mnt; mkdir -p /mnt/baadalcse;'"
		#ssh root@$ip 'mount filer03:/vol/baadalcse /mnt/baadalcse'
		val=`su www-data -c "ssh root@$ip 'cat /etc/crontab | grep \"\*\/45 \* \* \* \* root python /root/check_host_status.py\" | wc -l'"`
		echo $val
		if [[ $val -eq 0 ]]; then

			su www-data -c "ssh root@$ip 'echo \"*/45 * * * * root python /root/check_host_status.py\" >> /etc/crontab'"
		fi
	done
